...
with torch.autograd.set_detect_anomaly(True):
    ...
    def train(hyp, opt, device):
        ...
        dataloader, dataset = create_dataloader(train_path, imgsz, batch_size, gs, faces, texture_size, vertices, opt,
                                                ...
                                                data_fraction=opt.data_fraction, data_sampling=opt.data_sampling)
        ...
        net = torch.hub.load('yolov3',  'custom', 'weights/yolov3_9_5.pt', source='local')
        ...
        #     print(f"batch.shape:{batch.shape}")  
            ...
            for i, (imgs, texture_img, masks,imgs_cut, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
                ...
                # Run YOLO visualization every batch but silence its internal prints
        ...
        # end training
            ...
            if rank in [-1, 0]:
                logger.info(f"Images saved under {log_dir}")
        ...
        if opt.name:
            dest_path = texture_dir / f'texture_{opt.name}.npy'
        else:
            def _next_texture_idx(directory: Path):
                max_idx = 0
                for p in directory.glob('texture_train_*.npy'):
                    try:
                        ...
                        continue
                return max_idx + 1
            next_idx = _next_texture_idx(texture_dir)
            dest_path = texture_dir / f'texture_train_{next_idx}.npy'
            
        shutil.copyfile(os.path.join(log_dir, 'texture.npy'), dest_path)
    ...
    def make_log_dir(logs):
        global log_dir
        # Original verbose naming retained for reference:
        # dir_name = 'logs/' + ''.join(f"{k}-{v}+" for k, v in logs.items())
        # Simplified directory naming using incremental path for readability
        base = Path('logs') / 'exp'
        dir_path = increment_path(base, exist_ok=True)
    ...
    if __name__ == '__main__':
        ...
        # ------------------------------------#
        parser.add_argument('--weights', type=str, default='weights/yolov3_9_5.pt', help='initial weights path')
        ...
        parser.add_argument("--w_det", type=float, default=1.0, help="weight for detection loss")
        parser.add_argument('--epochs', type=int, default=10)
        parser.add_argument('--name', type=str, default='', help='Optional name for the experiment output')
        ...
        parser.add_argument('--data_fraction', type=float, default=1.0, help='percentage of data to use (0.0-1.0)')
        parser.add_argument('--data_sampling', type=str, default='random', help='sampling method: random or sequential')
