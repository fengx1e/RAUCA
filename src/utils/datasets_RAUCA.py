...
def create_dataloader(path, imgsz, batch_size, stride,faces, texture_size, vertices, opt, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
                      ...
                      data_fraction=1.0, data_sampling='random'):
    ...
    with torch_distributed_zero_first(rank):
        dataset = LoadImagesAndLabels(path, faces, texture_size, vertices,imgsz, batch_size,
                                      ...
                                      data_fraction=data_fraction, data_sampling=data_sampling)
...
def img2label_paths(img_paths, phase='training'):
    ...
    # Auto-detect pattern in the first path to decide replacement
    img_paths = list(img_paths) # Convert dict_keys to list for indexing
    if not img_paths: return []
    
    sample_path = img_paths[0]
    sa, sb = os.sep + 'train_new' + os.sep, os.sep + 'train_label_new' + os.sep
    
    # If path contains 'test_new', verify if we should switch to test labels
    if (os.sep + 'test_new' + os.sep) in sample_path:
        sa, sb = os.sep + 'test_new' + os.sep, os.sep + 'test_label_new' + os.sep
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, faces, texture_size, vertices, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        try:
            ...
            # --- Custom Data Fraction Logic ---
            if 0.0 < data_fraction < 1.0:
                total_imgs = len(self.img_files)
                num_to_keep = max(1, int(total_imgs * data_fraction))
                if num_to_keep > 0:
                    if data_sampling == 'random':
                        random.shuffle(self.img_files)
                        # Re-sort is not needed if we want random, but if order matters for other reasons (e.g. valid checks)
                        # usually for training simply random subset is fine. 
                        # However, to be safe with any downstream indexing that assumes order,
                        # we might want to slice then strict sort?
                        # For now, just slice.
                        self.img_files = self.img_files[:num_to_keep]
                    else: # sequential
                        self.img_files = self.img_files[:num_to_keep]
                    
                    logging.info(f"{prefix}Using {data_fraction*100}% of data: {len(self.img_files)}/{total_imgs} images ({data_sampling} sampling)")
            # ----------------------------------
